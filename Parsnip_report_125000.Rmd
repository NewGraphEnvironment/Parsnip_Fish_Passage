...
@data_fish_obs indicate that fish have not previously been recorded in the stream.  The watershed upstream of the crossing drains an area of `r my_watershed %>% pull(area_km)`km^2^ from an maximum elevation of approximately 1600m.  There are no other stream crossing (culvert) barriers on the mainstem of the stream upstream of the `r my_overview_info %>% pull(road_name)` although one crossing was modelled as located on a tributary upstream. 



<br>

Following office review, PSCIS stream crossing `r my_site` was ranked as a high priority for follow up with habitat confirmation due to the potential for rehabilitation of the crossing to provide access to upstream wetland (`r my_overview_info %>% pull(upstr_awet_gross_all)`ha) and instream (`r my_overview_info %>% pull(uphab_gross_sub22)`km) habitat and because it was ranked by @hooftPrinceGeorgeForest as a high priority for follow up (Table \@ref(tab:table-planning)). A map of the watershed including areas surveyed is provided in Attachment 1 – Map `r my_overview_info %>% pull(dbm_mof_50k_grid_map_tile)`.

<br>

**Stream Characteristics at Crossing**
The habitat confirmation was completed on `r date`.   At the time of the survey, the culvert  was considered a barrier to upstream fish passage with a length of ~`r my_pscis_info %>% pull(length_or_width_meters)`m, a culvert slope of `r my_pscis_info %>% pull(culvert_slope_percent)`m and an outlet drop of `r my_pscis_info %>% pull(outlet_drop_meters)`m (Table \@ref(tab:tableculvert), Figure \@ref(fig:photo-outlet)). There was a large, deep pool (`r my_pscis_info %>% pull(outlet_pool_depth_0_01m)`m) at the outlet of the culvert indicative of a severely confined channel.
...
caption = paste0('Aerial view of PSCIS crossing ', my_site, '.')
...
```{r photo-downstream1, fig.cap= caption, eval=F}
caption = paste0('View of habitat immediately downstream of PSCIS crossing ', my_site, '.')

img <- jpeg::readJPEG(paste0('data/photos/', my_site, '/DSCF4502.JPG'))
grid::grid.raster(img)
```

<br>

```{r photo-downstream2, fig.cap= caption, eval=F}
caption = paste0('View of habitat downstream of PSCIS crossing ', my_site, '.')

img <- jpeg::readJPEG(paste0('data/photos/', my_site, '/DSCF4508.JPG'))
grid::grid.raster(img)
```

<br>

```{r photo-downstream3, fig.cap= caption, eval=F}
caption = paste0('View of habitat downstream of PSCIS crossing ', my_site, '.')

img <- jpeg::readJPEG(paste0('data/photos/', my_site, '/DSCF4511.JPG'))
grid::grid.raster(img)
```
